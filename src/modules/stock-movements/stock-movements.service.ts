...
export class StockMovementsService {
  ...
  async create(data: any, userId: string) {
    ...
    if (data.type === 'in') {
      ...
      // Criar transação financeira (conta a pagar)
      try {
        // Buscar categoria 'stock_entry' do tipo 'expense'
        const categorias = await this.categoriesService.findAll();
        let categoriaStockEntry: Category | undefined = categorias.find(
          c => c.name.toLowerCase() === 'stock_entry' && c.type === 'expense'
        );
        if (!categoriaStockEntry) {
          categoriaStockEntry = await this.categoriesService.create({
            name: 'Stock Entry',
            type: 'expense'
          }, userId);
        }
        if (!categoriaStockEntry) {
          throw new Error('Falha ao criar ou localizar a categoria "Stock Entry" do tipo "expense".');
        }
        ...
        await this.transactionsService.create({
          ...
          category: categoriaStockEntry.id,
          dueDate,
          status: 'pending',
