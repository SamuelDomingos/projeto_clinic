...
function ScheduleView({
  ...
  const detectTimeConflicts = (appointments: AttendanceSchedule[], hours: string[], weekDays: string[], startOfGridWeek: Date, blockInterval: number) => {
    ...
    appointments.forEach((appointment, index) => {
      ...
      const key = `${position.dayIndex}-${Math.floor(position.topPosition / 40)}`; // Agrupar por dia e slot de tempo
  ...
  const calculateAppointmentPositionWithConflicts = (
    ...
    const basePosition = calculateAppointmentPosition(appointment, hours, weekDays, startOfGridWeek, blockInterval);
    if (!basePosition) return null;
    
    const key = `${basePosition.dayIndex}-${Math.floor(basePosition.topPosition / 40)}`;
    ...
    const appointmentIndex = conflictGroup.findIndex(a => a.id === appointment.id);
        ...
        <div className="absolute inset-0 pointer-events-none">
          ...
          <div className="relative h-full">
            {(() => {
              ...
              const appointmentConflicts = detectTimeConflicts(regularAppointments, hours, weekDays, startOfGridWeek, blockInterval);
              const blockedConflicts = detectTimeConflicts(blockedAppointments, hours, weekDays, startOfGridWeek, blockInterval);
              ...
              const blockedElements = sortedBlocked.map((appointment, index) => {
                const position = calculateAppointmentPositionWithConflicts(
                  appointment, hours, weekDays, startOfGridWeek, blockInterval, blockedConflicts
                );
...
return (
                  <div
                    ...
                    onMouseEnter={() => {
                      if (hasConflicts) {
                        const currentElement = document.querySelector(`[key="blocked-${appointment.id}"]`) as HTMLElement;
                        const conflictGroup = blockedConflicts[`${position.dayIndex}-${Math.floor(position.topPosition / 40)}`];
                        ...
                        if (currentElement && conflictGroup) {
                          ...
                          conflictGroup.forEach((conflictAppointment, idx) => {
                            if (conflictAppointment.id !== appointment.id) {
                              const siblingElement = document.querySelector(`[key="blocked-${conflictAppointment.id}"]`) as HTMLElement;
                              if (siblingElement) {
                                siblingElement.style.width = '20px';
                                siblingElement.style.left = `calc(100px + (100% - 100px) / ${weekDays.length} * ${position.dayIndex} + 4px + ((100% - 100px) / ${weekDays.length} - 8px) * 0.85 + ${idx * 22}px)`;
                    ...
                    onMouseLeave={() => {
                      if (hasConflicts) {
                        ...
                        if (conflictGroup) {
                          ...
                          conflictGroup.forEach((conflictAppointment, idx) => {
                            const element = document.querySelector(`[key="blocked-${conflictAppointment.id}"]`) as HTMLElement;
                            if (element) {
                              const originalWidthPercentage = 100 / conflictGroup.length;
                              const originalOffsetPercentage = idx * originalWidthPercentage;
                    ...
                    data-slot-key={`blocked-${position.dayIndex}-${Math.floor(position.topPosition / 40)}`}
                  >
                    ...
                    <div className="p-1.5 h-full flex flex-col justify-between group-hover:p-2 transition-all duration-300">
                      <div className="flex-1 min-h-0">
                        <div className="text-xs font-bold leading-tight mb-1 truncate group-hover:whitespace-normal flex items-center gap-1 transition-all duration-300" style={{ color: themeColors.text }}>
                        ...
                        <div className="text-xs font-bold leading-tight mb-1 truncate group-hover:whitespace-normal transition-all duration-300" style={{ color: themeColors.textMuted }}>
                          <span className="group-hover:font-medium">Profissional:</span> {doctorName}
                        </div>
                        {appointment.observation && (
                          <div className="text-xs font-bold leading-tight mb-1 truncate group-hover:whitespace-normal mt-1 transition-all duration-300" style={{ color: themeColors.textMuted }}>
                            <span className="group-hover:font-medium">Observação:</span> {appointment.observation}
                          </div>
                        )}
                      </div>
              ...
              const regularElements = sortedRegular.map((appointment, index) => {
                const position = calculateAppointmentPositionWithConflicts(
                  appointment, hours, weekDays, startOfGridWeek, blockInterval, appointmentConflicts
                );
                ...
                return (
                  <div
                    ...
                    onMouseEnter={() => {
                      if (hasConflicts) {
                        const currentElement = document.querySelector(`[key="appointment-${appointment.id}"]`) as HTMLElement;
                        const conflictGroup = appointmentConflicts[`${position.dayIndex}-${Math.floor(position.topPosition / 40)}`];
                        ...
                        if (currentElement && conflictGroup) {
                          ...
                          conflictGroup.forEach((conflictAppointment, idx) => {
                            if (conflictAppointment.id !== appointment.id) {
                              const siblingElement = document.querySelector(`[key="appointment-${conflictAppointment.id}"]`) as HTMLElement;
                              if (siblingElement) {
                                siblingElement.style.width = '20px';
                                siblingElement.style.left = `calc(100px + (100% - 100px) / ${weekDays.length} * ${position.dayIndex} + 4px + ((100% - 100px) / ${weekDays.length} - 8px) * 0.85 + ${idx * 22}px)`;
                                siblingElement.style.opacity = '0.7';
                    ...
                    onMouseLeave={() => {
                      if (hasConflicts) {
                        ...
                        if (conflictGroup) {
                          // Restaurar todas as posições e larguras originais
                          conflictGroup.forEach((conflictAppointment, idx) => {
                            const element = document.querySelector(`[key="appointment-${conflictAppointment.id}"]`) as HTMLElement;
                            if (element) {
                              const originalWidthPercentage = 100 / conflictGroup.length;
                              const originalOffsetPercentage = idx * originalWidthPercentage;
                              element.style.width = `calc(((100% - 100px) / ${weekDays.length} - 8px) * ${originalWidthPercentage} / 100)`;
                    ...
                    data-slot-key={`appointment-${position.dayIndex}-${Math.floor(position.topPosition / 40)}`}
                  >
                    {/* Card normal com conteúdo expandido no hover */}
                    <div className="p-1.5 h-full flex flex-col justify-between group-hover:p-2 transition-all duration-300">
                      <div className="flex-1 min-h-0">
                        ...
                        <div className="text-xs font-bold leading-tight mb-1 truncate group-hover:whitespace-normal transition-all duration-300" style={{ color: themeColors.textMuted }}>
                          <span className="group-hover:font-medium">Paciente:</span> {patientName}
                        </div>
                        {appointment.serviceSession?.name && (
                          <div className="text-xs font-bold leading-tight mb-1 transition-all duration-300" style={{ color: themeColors.textMuted }}>
                            <span className="group-hover:font-medium">Serviço:</span> {appointment.serviceSession.name}
                          </div>
                        )}
                        {appointment.observation && (
                          <div className="text-xs font-bold leading-tight mb-1 transition-all duration-300" style={{ color: themeColors.textMuted }}>
