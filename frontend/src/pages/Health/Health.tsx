...
// Definir tipo Category para uso local
interface Category {
  id: string | number;
  title?: string;
  name?: string;
...
export default function Health() {
    ...
    if (user?.id && categories.length > 0) {
      const fetchAll = async () => {
        ...
        for (const category of categories) {
          if (Array.isArray(category.questionnaires)) {
            await Promise.all(
              category.questionnaires.map(async (q) => {
                const resp = await responseService.getUserResponse(
                  Number(q.id),
                  user.id
                );
                newRespondidos[q.id] = !!resp;
              })
            );
          }
        }
  ...
  // Filtrar questionários baseado na pesquisa
  const filteredCategories = categories.map((category) => {
    const questions = Array.isArray(category.questionnaires)
      ? category.questionnaires
      : [];
    const catTitle = (category.title || category.name || "").toLowerCase();
    return {
      ...category,
      questionnaires: questions.filter(
        (questionario) =>
          (questionario.name || "")
            .toLowerCase()
          ...
          <div className="text-center mb-6">
            <p className="text-gray-400">
              {filteredCategories.reduce(
                (total, cat) => total + cat.questionnaires.length,
                0
              )}{" "}
          ...
          <div className="max-w-4xl mx-auto">
            <Accordion type="single" collapsible className="space-y-4">
              {filteredCategories
                .filter(
                  (category) =>
                    category.questionnaires.length > 0 || !searchQuery
                )
                .map((category) => {
                  const iconName =
                    typeof category.icon === "string" ? category.icon : "brain";
                  const IconComponent = iconMap[iconName] || Brain;
                  const color = colorMap[category.color as string] || "blue";
                  const categoryProgress = getCategoryProgress(
                    category.questionnaires.map((q) => String(q.id))
                  );
                      ...
                      >
                        <AccordionTrigger className="hover:no-underline p-0">
                          <div className="flex flex-row items-start gap-4 w-full p-2">
                            <div
                              className={`p-3 rounded-full bg-${color}-500/20 flex items-center justify-center flex-shrink-0`}
                            >
                              <IconComponent
                                size={24}
                        ...
                        <AccordionContent>
                          <CardContent className="pt-0">
                            {category.questionnaires.length === 0 ? (
                              <p className="text-gray-500 text-center py-4">
                                Nenhum questionário encontrado para "
                                {searchQuery}"
                              </p>
                            ) : (
                              <div className="grid gap-3">
                                {category.questionnaires.map((questionario) => {
                                  const jaRespondeu =
                                    respondidos[questionario.id];
                                  return (
                                    <div
                                      key={questionario.id}
            ...
            {searchQuery &&
              filteredCategories.every(
                (category) => category.questionnaires.length === 0
              ) && (
